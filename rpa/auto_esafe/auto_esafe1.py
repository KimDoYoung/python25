...
def find_and_click(image_path, region=None, grayscale=True, confidence=0.8):
...
def find_and_press_key(image_path: str, key: str, region: Optional[tuple] = None, grayscale: bool = True, confidence: float = 0.8) :
    """
...
def move_and_click(image_path: str, region: Optional[Tuple[int, int, int, int]] = None, grayscale: bool = True, confidence: float = 0.8, duration: float = 0.5) -> bool:
    """
...
def main():
    ...
    # 로그인 버튼 클릭
    find_and_click('./images/login_button.png', region=region)
    ...
    # 인증서 선택 버튼 클릭
    find_and_click('./images/certi_select_button.png', grayscale=True)
    ...
    # 조회 버튼 클릭
    region = get_region(RegionName.RIGHT_TOP)
    find_and_click('./images/query.png', region=region)
    ...
    find_and_click('./images/download_options.png', region=region, grayscale=True)
    ...
    # 조회버튼 클릭
    region = get_region(RegionName.RIGHT_TOP)
    move_and_click('./images/query.png', region=region)
    time.sleep(3)
    # 기준가 조회 체크까지 기다림
    query_finish_check = wait_for_image('./images/query_finish_check.png', region=region)
    time.sleep(3)
    ...
    time.sleep(1)
    pyautogui.press('enter')
    log.info(f"파일 저장 경로(기준가2): {saved_file_path}")
