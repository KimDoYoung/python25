...
class VariableManager:
    ...
    def set_variable(self, var_name, value:Token, local=False):
        """변수 설정 (지역/전역 변수 구분), 저장도 Token으로 변경 """
        var_name = var_name.upper()  # ✅ 대소문자 무시
        # value = self._convert_value(value)  # ✅ Boolean 및 NULL 변환 적용

        if local and self.local_vars_stack:
            self.local_vars_stack[-1][var_name] = value  # 지역 변수 저장
        else:
            self.global_vars[var_name] = value  # 전역 변수 저장
