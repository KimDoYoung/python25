...
class PrintCommand(BaseCommand):
    def execute(self, args: Token, executor):
        if not args:
        ...
        def evaluate_match(match):
            ...
            return str(evaluator.evaluate(expression))  # 평가 후 문자열 반환
...
class PrintCommand(BaseCommand):
    def execute(self, args: list[Token], executor):
        ...
        output = " ".join(token.value for token in args)
